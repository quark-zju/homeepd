...
#define MAX_HTTP_OUTPUT_BUFFER (194000)
    ...
    case HTTP_EVENT_ON_DATA:
        ...
        if (!esp_http_client_is_chunked_response(evt->client)) {
            ...
            if (evt->user_data) {
                ...
                // ESP_LOGI(TAG, "Copy to user_data buffer, len=%d", copy_len);
                if (copy_len) {
                ...
                // ESP_LOGI(TAG, "Copy to user_data buffer 2, len=%d", copy_len);
                if (copy_len) {
